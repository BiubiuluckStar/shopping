...
export default {
  ...
  methods: {
    ...
    async open() {
      // 生成二维码，使用qrcode插件QRCode.toDataURL(图片地址)
      // 返回的是一个promise对象，使用async与await得到结果
      ...
      // 定时向服务器发请求，寻求是否支付成功，若支付成功则关闭定时器，关闭弹框，跳转路由
      //如果没有定时器，开启一个新的定时器
      if (!this.timer) {
